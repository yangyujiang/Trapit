...
EnemyView::EnemyView():_insect(NULL),_insectModel(NULL),pViewDelegate(NULL),animationAction(NULL)
{
}
...
bool EnemyView::init(GameViewDelegate* pViewDelegate,Enemy* gameInsect){
	bool pRet=false;
	do{ 
		// 先调用超类的init方法
        CC_BREAK_IF(! CCLayer::init());
...
void EnemyView::update(float dt){
	...
	_insect->setPosition(_insectModel->getCenterPoint());
	_insect->setRotation(_insectModel->getRotation());
	if(!_insectModel->getAlive()){
		this->stopAllActions();
		_insect->stopAllActions();
		this->removeChild(_insect,true);
...
void EnemyView::testModelDelegate(){
	//CCLog("testModelDelegate");
	update(0);
}
...
void EnemyView::playAnimation(float velocity){
	if(animationAction!=NULL){
		this->stopAction(animationAction);
		animationAction=NULL;
	}
